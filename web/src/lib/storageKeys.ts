...
// Кэш данных и очередь удалений — свои у каждого аккаунта. Раньше ключи были
// общими: после переключения аккаунта при сбое сети из кэша поднимались
// чужие заказы, а очередь удалений одного аккаунта «исполнялась» под другим
// (RLS молча возвращал ноль строк, и удаление считалось выполненным).
export function accountKey(base: string, userId: string | null | undefined): string {
  return `${base}::${userId || "local"}`
}

// Снимок последнего известного состояния облака (что отправлено, updated_at
// каждой строки). Без него после сбоя загрузки локальный кэш считался целиком
// «новым» и при первой же синхронизации перезаписывал более свежее облако.
export const CLOUD_SNAPSHOT_KEY = "design_crm_cloud_snapshot_v1"
