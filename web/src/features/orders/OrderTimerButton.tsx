...
export function OrderTimerButton({ order, size = "sm" }: { order: Order; size?: "sm" | "md" }) {
  ...
  return (
    <button
      ...
      className={
        size === "md"
          ? "flex shrink-0 items-center gap-1.5 rounded-lg bg-emphasis/85 px-3 py-1.5 text-xs font-extrabold text-emphasis-foreground hover:bg-emphasis"
          : "flex shrink-0 items-center gap-1 rounded-full bg-emphasis/85 px-2 py-1 text-2xs font-extrabold text-emphasis-foreground"
      }
    >
