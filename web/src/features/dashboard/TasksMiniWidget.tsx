  ...
  return (
    <div className="glass-surface flex flex-col rounded-xl p-4.5">
      <div className="mb-3 flex items-center justify-between">
        <div className="text-[14px] font-bold">Задачи</div>
      ...
      <div className="flex flex-col gap-1">
        {active.length === 0 && <div className="py-5 text-center text-[12.5px] text-muted-foreground">На сегодня задач нет</div>}
        {active.map((t) => {
          ...
          return (
            <div key={t.id} className="group flex items-start gap-2 rounded-lg px-2 py-1.5 hover:bg-muted">
              ...
              />
              <div className="min-w-0 flex-1 text-[13px] leading-snug break-words">
                {overdue && <span className="mr-1 text-[10.5px] font-bold text-destructive" title={`Со дня ${t.createdAt}`}>⏰</span>}
      ...
      {done.length > 0 && (
        <div className="mt-2 border-t border-border pt-2">
          <div className="flex items-center justify-between text-[11.5px] font-bold text-muted-foreground">
            ...
            <div className="mt-1 flex flex-col gap-0.5">
              {done.map((t) => (
                <div key={t.id} className="group flex items-start gap-2 rounded-lg px-2 py-1 hover:bg-muted">
                  ...
                  <div className="min-w-0 flex-1 text-[12.5px] leading-snug break-words text-muted-foreground line-through"><Linkified text={t.text} /></div>
                  <button type="button" onClick={() => remove(t.id)} className="mt-0.5 opacity-0 group-hover:opacity-100" title="Удалить">
                    <Trash2 className="size-3.5 text-muted-foreground hover:text-destructive" />
                  </button>
                </div>
      ...
      <div className="mt-3 flex shrink-0 gap-1.5 border-t border-border pt-3">
        <input
          ...
          className="h-8 min-w-0 flex-1 rounded-md border border-border bg-background px-2.5 text-[12.5px] outline-none focus:border-ring"
        />
        <button
          type="button"
          onClick={addTask}
      ...
      {upcoming.length > 0 && (
        <div className="mt-3.5 shrink-0 border-t border-border pt-3.5">
          ...
          <div className="flex flex-col gap-1">
            {upcoming.map((t) => (
              <div key={t.id} className="flex items-start gap-2 px-2 py-1 text-[12.5px]">
                <span className="mt-px shrink-0 rounded-md bg-overlay/10 px-1.5 py-0.5 text-[10px] font-bold text-foreground/70">{UPCOMING_LABELS[t.period || "today"]}</span>
