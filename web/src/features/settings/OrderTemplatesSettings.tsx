...
export function OrderTemplatesSettings() {
  ...
  return (
    <div className="glass-surface rounded-xl p-4.5">
      <h3 className="text-lg font-bold">Шаблоны заказов</h3>
      <div className="mb-3 text-xs text-muted-foreground">Готовые наборы позиций для быстрого создания заказа.</div>

      <div className="flex flex-col gap-2">
        {templates.length === 0 && <div className="text-sm text-muted-foreground">Шаблонов пока нет.</div>}
        {templates.map((t) => (
          <div key={t.id} className="flex items-center justify-between gap-2 rounded-lg bg-muted px-3 py-2">
            <div className="min-w-0">
              ...
              <div className="truncate text-xs text-muted-foreground">{t.lines.map((l) => l.label || "?").join(", ") || "нет позиций"}</div>
            </div>
